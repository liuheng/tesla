#include <tesla/base/Time.h>
#include <tesla/base/Date.h>
...
bool readTimeFile(const char* zonefile, struct Time::Data* data)
{
    File f(zonefile);
    if (f.valid())
    {
...
const LocalTime* findLocalTime(const Time::Data& data, Transition sentry, TimeComparer comp)
{
...
Time::Time(const char* zonefile)
    : data_(new Time::Data)
{
    if (!readTimeFile(zonefile, data_.get()))
    {
        data_.reset();
    }
}

Time::Time(int eastOfUtc, const char* name)
    : data_(new Time::Data)
{
...
struct tm Time::toLocalTime(time_t seconds) const
{
    struct tm localTime;
    ...
    const LocalTime* local = findLocalTime(data, sentry, TimeComparer(true));
...
struct tm Time::toUtcTime(time_t secondsSinceEpoch, bool yday)
{
    struct tm utc;
    bzero(&utc, sizeof(utc));
    utc.tm_zone = "GMT";
    int seconds = static_cast<int>(secondsSinceEpoch % SECONDSPERDAY);
    int days = static_cast<int>(secondsSinceEpoch / SECONDSPERDAY);
    if (seconds < 0)
    {
    ...
    fillHMS(seconds, &utc);
    Date date(days + Date::JULIAN_DAY_OF_1970_01_01);
...
time_t Time::fromLocalTime(const struct tm& localTm) const
{
    assert(data_ != NULL);
    const Data& data(*data_);
    ...
    const LocalTime* local = findLocalTime(data, sentry, TimeComparer(false));
    if (localTm.tm_isdst)
    {
...
time_t Time::fromUtcTime(const struct tm& utc)
{
    return fromUtcTime(utc.tm_year + 1900, utc.tm_mon + 1, utc.tm_mday,
                       utc.tm_hour, utc.tm_min, utc.tm_sec);
}

time_t Time::fromUtcTime(int year, int month, int day,
                         int hour, int minute, int seconds)
{
    Date date(year, month, day);
    int secondsInDay = hour * 3600 + minute * 60 + seconds;
    time_t days = date.julianDayNumber() - Date::JULIAN_DAY_OF_1970_01_01;
    return days * SECONDSPERDAY + secondsInDay;
}
